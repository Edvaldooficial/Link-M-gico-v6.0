...
// Limita texto a ~260 caracteres (~3 linhas em média)
function summarizeToThreeLines(text) {
    if (!text) return "";
    const clean = text.replace(/\s+/g, " ").trim();
    if (clean.length <= 260) return clean;
    return clean.substring(0, 257) + "...";
}
...
// Elementos DOM
const elements = {
    extractedSummary: document.getElementById('extracted-summary'),
...
// Vincular botão "Ativar Chatbot"
document.addEventListener("DOMContentLoaded", () => {
    const btn = document.getElementById("activate-chatbot");
    if (btn) {
        btn.addEventListener("click", navigateToChat);
    }
});

document.addEventListener("DOMContentLoaded", function(){
    const btn = document.getElementById("activate-chatbot");
    if(btn){ btn.addEventListener("click", navigateToChat); }
});

// Explicit reference for validation
console.log('Hooking #activate-chatbot');
// Explicit reference for validation
console.log('Hooking #chat-section');
